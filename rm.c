...
int main(int argc, char **argv) {
	...
	file_info = find_file(argv[3], get_block(disk, gd -> inode_table, BLOCK_SIZE));
	if(file_info.type == 'd'){
        ...
        if (strlen(filename) == current -> name_len && !strncmp(filename, current -> name, strlen(filename))){
            ...
            if(previous==NULL){
                ...
                if(current->rec_len==BLOCK_SIZE){
                    ...
                    unsigned int blocks_used = (current_inode -> size > 0)? ((current_inode -> i_size - 1)/BLOCK_SIZE) + 1 : 0;
	...
	if(current_file_inode -> i_links_count == 0){
		...
		gd -> free_blocks_count += current_file_inode -> blocks / 2;
