...
struct basic_fileinfo search_directory(char *inode_base, unsigned int inode_num, struct parsed_filename *filename){
	...
	while(cur_index_serviced < current_inode -> size){
		...
		if (cur_index_serviced < current_inode->size && cur_index_serviced % 1024 == 0){
			cur_inode_index++;
...
char *get_block_from_inode(char *inode_base, unsigned int inode_num, unsigned int inode_index){
	...
	if (inode_index < 12){
		return get_block(disk, current_inode -> block[inode_index], 1024);
	}
	//assuming up to single indirect pointers!
	else{
		unsigned int *indirect_pointer_block = (unsigned int*) get_block(disk, current_inode -> block[12], 1024);
...
int add_block_in_inode(struct ext2_inode *current_inode, char *block_bitmap, unsigned int inode_num){
	unsigned int new_inode_block_index = (current_inode -> size > 0)? ((((current_inode -> size) - 1) >> 10) + 1) : 0;
	...
	if (new_inode_block_index <= 12) {
		current_inode -> block[new_inode_block_index] = new_block + 1;
		set_bit(block_bitmap, new_block);
	}
	if(new_inode_block_index == 12) {
		new_block = find_first_free_bit(block_bitmap, 0, 128);
		if(new_block == -1){
			unset_bit(block_bitmap, current_inode -> block[12] - 1);
			return -1;
		}
	}
	if(new_inode_block_index >= 12) {
...
void set_inode(struct ext2_inode *inode, unsigned int inode_num, unsigned int size, unsigned *blocks, int num_blocks, char *block_bitmap, char *inode_bitmap, unsigned int mode){
	...
	unsigned int i;
	inode -> size = size;
	inode -> blocks = (inode -> size > 0)? 2*((inode -> size - 1)/ 1024) + 2: 0;
	if(inode -> blocks >= 25){
		inode -> blocks += 2;
	}
	inode -> mode = 0x1ff | mode;
	if(num_blocks > 12){
		inode -> block[12] = blocks[num_blocks - 1] + 1;
...
void set_block_in_inode(struct ext2_inode* inode, unsigned block_number, unsigned inode_index, char *block_bitmap){
	if(inode_index < 12){
		inode -> block[inode_index] = block_number;
	}
	else{
		unsigned int* indirect_pointer_block = (unsigned int *) get_block(disk, inode -> block[12], 1024);
...
void free_inode(struct ext2_inode *current_inode, void *disk, char *blockbitmap){
    unsigned int i_size = current_inode->size;
    ...
    for(block=0; block<blocks; block++){
        ...
        if(block>=12){
            ...
            unset_bit(blockbitmap, current_inode->block[block] - 1);
            //free(current_inode->i_block[block-1]);
        }
        
    }
	if(blocks > 12){
		unset_bit(blockbitmap, current_inode->block[12] - 1);
    }
}
